...
- **`OnboardingTour`** (`components/onboarding/OnboardingTour.tsx`): a fixed, full-screen modal (`fade-scale-in`-style overlay) rendered conditionally by `AppShell` — it self-fetches nothing, the parent decides visibility from `user_profiles.onboarding_completed` and passes `userId`/`onComplete`. Also mounted directly by `Sidebar` ("Take the Tour") and `ProfileTab` ("Replay the app tour") for manual replay. If you add another place that should trigger it, reuse this component rather than rolling a second modal.
