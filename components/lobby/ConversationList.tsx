  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 px-8 py-16 text-center">
      ...
      <p className="text-sm text-neutral-400 leading-relaxed max-w-xs">
        No conversations yet. Jump into a <strong className="text-neutral-500">Room</strong> to chat with the group, or open a{' '}
        <strong className="text-neutral-500">DM</strong> from the People tab.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 py-3 border-b border-neutral-200 bg-white">
        ...
        <p className="text-xs text-neutral-400 mt-0.5">Your recent DMs and room activity</p>
      </div>
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-neutral-100 transition-colors text-left border-b border-neutral-100"
              >
                {/* Avatar / Icon */}
                <div className="relative flex-shrink-0">
                  {item.type === 'room' && item.icon ? (
                    <div className="w-10 h-10 rounded-full bg-neutral-200 flex items-center justify-center text-xl">
                      {item.icon}
                    </div>
                  ) : (
                    <div className="w-10 h-10 rounded-full bg-brand-green flex items-center justify-center text-white text-sm font-semibold">
                      {initial}
                    </div>
                  )}
                  {item.type === 'room' && (
                    <span className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-brand-blue rounded-full flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={`text-sm font-medium truncate ${unread > 0 ? 'text-neutral-900' : 'text-neutral-700'}`}>
                      {item.name}
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                    <p className={`text-xs truncate max-w-[170px] ${unread > 0 ? 'text-neutral-600 font-medium' : 'text-neutral-400'}`}>
                      {item.last_message}
                    </p>
