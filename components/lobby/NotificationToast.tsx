...
export function NotificationToast({ toast, index, onClose, onClick }: NotificationToastProps) {
  return (
    <div
      ...
      className="bg-white border border-gray-200 rounded-lg shadow-xl p-4 w-80
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="font-bold text-gray-900 text-sm">{toast.senderName}</span>
          {toast.type === 'room' && toast.roomName && (
            <span className="text-xs text-gray-500">in {toast.roomName}</span>
          )}
        </div>
        <p className="text-gray-600 text-sm truncate">{toast.message}</p>
      </div>
