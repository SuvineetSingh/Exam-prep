  ...
  return (
    <div className={`flex items-start gap-3 ${isOwnMessage ? 'flex-row-reverse' : ''}`}>
      ...
      <button
        ...
        className="w-8 h-8 rounded-full bg-brand-green flex-shrink-0 flex items-center justify-center text-white text-sm font-semibold hover:opacity-80 transition-opacity"
      >
        {initial}
      </button>
      ...
      <div className={`max-w-[70%] ${isOwnMessage ? 'items-end' : 'items-start'}`}>
        ...
        <div className={`flex items-center gap-1.5 mb-1 ${isOwnMessage ? 'justify-end' : ''}`}>
          <button
            onClick={() => onClickUser?.(message.sender_id)}
            className="text-xs font-semibold text-neutral-700 hover:text-brand-green transition-colors"
          >
            {message.sender?.username || 'Unknown'}
          </button>
          {message.sender?.is_bot && (
            <span className="text-[10px] text-neutral-400 bg-neutral-200 px-1.5 py-0.5 rounded-full leading-none">
              bot
            </span>
          )}
