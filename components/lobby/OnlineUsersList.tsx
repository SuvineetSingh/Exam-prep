    ...
    <div className="p-3">
      ...
      <div className="flex gap-1 px-1 mb-3">
        <button
          ...
          className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${
            findTab === 'online' ? 'bg-brand-green text-white' : 'text-neutral-400 hover:bg-neutral-200'
          }`}
        >
          Online — {users.length}
        </button>
        <button
          ...
          className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${
            findTab === 'find' ? 'bg-brand-green text-white' : 'text-neutral-400 hover:bg-neutral-200'
          }`}
        >
          Find People
        </button>
      ...
      {findTab === 'online' ? (
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left transition-colors ${
                    isCurrentUser ? 'opacity-60 cursor-default' : 'hover:bg-neutral-100'
                  }`}
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      {user.is_bot && (
                        <span className="text-[10px] text-neutral-400 bg-neutral-200 px-1.5 py-0.5 rounded-full leading-none">bot</span>
                      )}
                    </div>
                    {user.exam_type && (
                      <span className="text-xs text-neutral-500">{user.exam_type}</span>
                    )}
                  </div>
                </button>
              );
        ...
        <div>
          ...
          <p className="text-[11px] text-neutral-400 px-2 mb-3">Type at least 2 characters and press Go or Enter.</p>
          ...
          <div className="space-y-1">
            {searchResults.map((result) => {
              ...
              return (
                <div key={result.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-neutral-100">
                  <div className="w-8 h-8 rounded-full bg-brand-green flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">
                    {initial}
                  </div>
                  <div className="min-w-0 flex-1">
            ...
            {searchResults.length === 0 && searchQuery.trim().length >= 2 && !searching && (
              <p className="text-xs text-neutral-400 px-3 py-4 text-center">No users found for "{searchQuery}"</p>
            )}
          </div>
        </div>
      )}
