    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <h3 className="text-sm font-bold text-gray-900">Pinned Rooms</h3>
            <p className="text-xs text-gray-500 mt-0.5">Select up to {MAX_PINS} rooms to pin to the top</p>
          </div>
          <button
            onClick={onClose}
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-left ${
                  isPinned
                    ...
                    : disabled
                    ? 'bg-gray-50 border-2 border-transparent opacity-40 cursor-not-allowed'
              ...
              >
                ...
                <span className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                  isPinned ? 'border-amber-500 bg-amber-500' : 'border-gray-300'
                }`}>
                  {isPinned && (
        ...
        <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
          <span className="text-xs text-gray-400">{pinnedIds.length}/{MAX_PINS} selected</span>
          <button
            onClick={onClose}
